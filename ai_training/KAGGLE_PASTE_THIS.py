"""
╔══════════════════════════════════════════════════════════════════════════════╗
║   STRIKE-VECTOR AI GUIDANCE — KAGGLE TRAINING (PASTE INTO NOTEBOOK CELL)   ║
║                                                                            ║
║   Step 1: Enable GPU T4 x2 in Kaggle Settings → Accelerator               ║
║   Step 2: Copy ALL of this code into a single Kaggle cell                  ║
║   Step 3: Click Run                                                        ║
║   Step 4: Download trained_missile_brain.pth from Output section           ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
plt.rcParams.update({
    ...
    'figure.dpi': 120, 'font.size': 10, 'font.family': 'monospace',
})
CYAN, RED, GREEN, AMBER, PURPLE = '#00d4ff', '#ff4444', '#00ff66', '#ffaa00', '#a855f7'

print("=" * 70)
print("  STRIKE-VECTOR AI GUIDANCE — KAGGLE GPU TRAINING")
print("=" * 70)
...
if data_path is None and os.path.exists('training_dataset.npz'):
    data_path = 'training_dataset.npz'

assert data_path is not None, "❌ training_dataset.npz not found! Add it as Kaggle Dataset input."
...
STATE_LABELS = ['Rel Pos X', 'Rel Pos Y', 'Rel Pos Z',
...
plt.savefig('viz01_state_distributions.png', bbox_inches='tight'); plt.show()
print("  ✅ VIZ 1 done")
...
plt.savefig('viz02_action_distributions.png', bbox_inches='tight'); plt.show()
print("  ✅ VIZ 2 done")
...
plt.tight_layout()
plt.savefig('viz03_correlation_heatmap.png', bbox_inches='tight'); plt.show()
print("  ✅ VIZ 3 done")


# ============================================================================
# VIZ 4: Range Distribution
# ============================================================================
ranges = np.linalg.norm(states_raw[:, :3], axis=1)
fig, ax = plt.subplots(figsize=(10, 4))
ax.hist(ranges, bins=100, color=PURPLE, alpha=0.8, edgecolor='none')
ax.set_xlabel('Range to Target (m)'); ax.set_ylabel('Sample Count')
...
plt.savefig('viz04_range_distribution.png', bbox_inches='tight'); plt.show()
print("  ✅ VIZ 4 done")


# ============================================================================
# VIZ 5: Command Intensity vs Range
# ============================================================================
...
scatter = ax.scatter(ranges[sub]/1000, cmd_magnitude[sub], c=cmd_magnitude[sub], cmap='inferno', s=0.5, alpha=0.3)
ax.set_xlabel('Range (km)'); ax.set_ylabel('Command (G)')
ax.set_title('VIZ 5: Guidance Command Intensity vs Range', fontsize=14, color=CYAN, fontweight='bold')
plt.colorbar(scatter, ax=ax, label='G-Load'); plt.tight_layout()
plt.savefig('viz05_cmd_vs_range.png', bbox_inches='tight'); plt.show()
print("  ✅ VIZ 5 done")
...
# ============================================================================
# SECTION 4: MODEL (EXACT MATCH WITH ai_law.py)
# ============================================================================
print("\n🧠 Model Architecture (matches ai_law.py exactly):")

class MissileGuidanceNet(nn.Module):
    """Exact same architecture as physics/guidance/ai_law.py"""
...
print(f"  Layers: 12→256→512→512→256→128→3")
print(f"  Parameters: {total_params:,}")


# ============================================================================
# SECTION 5: OPTUNA — TUNE LR, WD, BATCH SIZE (architecture is fixed)
# ============================================================================
print("\n🔬 Optuna: Tuning learning rate, weight decay, batch size (30 trials)...")
...
# ============================================================================
# VIZ 6: Optuna History
# ============================================================================
fig, ax = plt.subplots(figsize=(12, 5))
vals = [t.value for t in study.trials if t.value is not None]
nums = [t.number for t in study.trials if t.value is not None]
ax.scatter(nums, vals, c=CYAN, s=20, alpha=0.7); ax.plot(nums, vals, color=CYAN, alpha=0.3)
...
ax.legend(); plt.tight_layout()
plt.savefig('viz06_optuna_history.png', bbox_inches='tight'); plt.show()
print("  ✅ VIZ 6 done")


# ============================================================================
# VIZ 7: Param Importance
# ============================================================================
fig, ax = plt.subplots(figsize=(10, 5))
try:
...
plt.tight_layout()
plt.savefig('viz07_param_importance.png', bbox_inches='tight'); plt.show()
print("  ✅ VIZ 7 done")


# ============================================================================
# SECTION 6: FULL 1000-EPOCH TRAINING
# ============================================================================
print("\n🚀 Training 1000 epochs with optimal hyperparameters...")
...
print(f"\n{'Epoch':>6} | {'Train':>12} | {'Val':>12} | {'LR':>10} | {'GradNorm':>10} | {'Time':>6}")
print("-" * 68)
t0 = time.time()
...
for epoch in range(1, 1001):
    ...
    if epoch % 50 == 0 or epoch == 1:
        print(f"{epoch:>6} | {avg_train:>12.6f} | {val_loss:>12.6f} | {lr_now:>10.2e} | {avg_gn:>10.4f} | {time.time()-t0:>5.0f}s")

total_time = time.time() - t0
print(f"\n{'='*68}")
print(f"  ✅ TRAINING COMPLETE! Time: {total_time:.0f}s ({total_time/60:.1f} min)")
print(f"  Best Epoch: {best_epoch} | Best Val Loss: {best_val_loss:.6f}")
print(f"{'='*68}")


# ============================================================================
# SECTION 7: POST-TRAINING EVALUATION & VISUALIZATIONS 8-20
# ============================================================================
...
ax.legend(); plt.tight_layout()
plt.savefig('viz08_loss_curve.png', bbox_inches='tight'); plt.show()
print("  ✅ VIZ 8 done")
...
plt.tight_layout()
plt.savefig('viz09_lr_schedule.png', bbox_inches='tight'); plt.show()
print("  ✅ VIZ 9 done")
...
ax.legend(); plt.tight_layout()
plt.savefig('viz10_gradient_norms.png', bbox_inches='tight'); plt.show()
print("  ✅ VIZ 10 done")


# VIZ 11: Predicted vs True Scatter
...
for i, (ax, name) in enumerate(zip(axes, axis_names)):
    ...
    ax.set_xlabel('True (G)'); ax.set_ylabel('Predicted (G)')
plt.tight_layout(rect=[0, 0, 1, 0.93])
plt.savefig('viz11_pred_vs_true.png', bbox_inches='tight'); plt.show()
print("  ✅ VIZ 11 done")
...
plt.tight_layout(rect=[0, 0, 1, 0.93])
plt.savefig('viz12_error_distributions.png', bbox_inches='tight'); plt.show()
print("  ✅ VIZ 12 done")


# VIZ 13: Error Cross-Correlation
fig, ax = plt.subplots(figsize=(8, 6))
ec = np.corrcoef(errors_real.T)
sns.heatmap(ec, ax=ax, xticklabels=axis_names, yticklabels=axis_names,
            cmap='RdBu_r', center=0, annot=True, fmt='.3f', vmin=-1, vmax=1)
ax.set_title('VIZ 13: Error Cross-Correlation', fontsize=14, color=CYAN, fontweight='bold')
plt.tight_layout()
plt.savefig('viz13_error_heatmap.png', bbox_inches='tight'); plt.show()
print("  ✅ VIZ 13 done")
...
plt.tight_layout()
plt.savefig('viz14_error_vs_range.png', bbox_inches='tight'); plt.show()
print("  ✅ VIZ 14 done")
...
plt.tight_layout(rect=[0, 0, 1, 0.95])
plt.savefig('viz15_weight_histograms.png', bbox_inches='tight'); plt.show()
print("  ✅ VIZ 15 done")


# VIZ 16: G-Load AI vs Expert
true_mag_g = np.linalg.norm(val_true_real, axis=1) / 9.81
pred_mag_g = np.linalg.norm(val_preds_real, axis=1) / 9.81
fig, ax = plt.subplots(figsize=(10, 5))
ax.hist(true_mag_g, bins=100, color=CYAN, alpha=0.5, label='Expert (APNG)', edgecolor='none')
ax.hist(pred_mag_g, bins=100, color=AMBER, alpha=0.5, label='AI Predicted', edgecolor='none')
ax.set_xlabel('G-Load'); ax.set_ylabel('Count')
ax.set_title('VIZ 16: G-Load Distribution — AI vs Expert', fontsize=14, color=CYAN, fontweight='bold')
ax.legend(); plt.tight_layout()
plt.savefig('viz16_gload_comparison.png', bbox_inches='tight'); plt.show()
print("  ✅ VIZ 16 done")


# VIZ 17: Error vs Target Evasion
tgt_accel_mag = np.linalg.norm(val_states_real[:, 9:12], axis=1) / 9.81
fig, ax = plt.subplots(figsize=(10, 5))
sub_t = np.random.choice(len(tgt_accel_mag), min(30000, len(tgt_accel_mag)), replace=False)
ax.scatter(tgt_accel_mag[sub_t], error_mag_g[sub_t], c=PURPLE, s=0.5, alpha=0.3)
ax.set_xlabel('Target G-Load'); ax.set_ylabel('AI Error (G)')
ax.set_title('VIZ 17: Error vs Target Evasion Intensity', fontsize=14, color=CYAN, fontweight='bold')
plt.tight_layout()
plt.savefig('viz17_error_vs_evasion.png', bbox_inches='tight'); plt.show()
print("  ✅ VIZ 17 done")


# VIZ 18: QQ Plots
fig, axes = plt.subplots(1, 3, figsize=(16, 4))
fig.suptitle('VIZ 18: Residual QQ Plots', fontsize=14, color=CYAN, fontweight='bold')
for i, (ax, name) in enumerate(zip(axes, axis_names)):
...
plt.tight_layout(rect=[0, 0, 1, 0.93])
plt.savefig('viz18_qq_plots.png', bbox_inches='tight'); plt.show()
print("  ✅ VIZ 18 done")
...
ax.set_xlabel('Error (G)'); ax.set_ylabel('CDF')
ax.set_title('VIZ 19: Cumulative Error Distribution', fontsize=14, color=CYAN, fontweight='bold')
plt.tight_layout()
plt.savefig('viz19_error_cdf.png', bbox_inches='tight'); plt.show()
print("  ✅ VIZ 19 done")


# VIZ 20: Final Dashboard
fig = plt.figure(figsize=(18, 10))
fig.suptitle('VIZ 20: STRIKE-VECTOR AI — FINAL PERFORMANCE DASHBOARD', fontsize=16, color=CYAN, fontweight='bold', y=0.98)
...
ax1.set_yscale('log'); ax1.set_title('Loss Curve', color=CYAN); ax1.legend(fontsize=8)
...
ax2.set_title(f'R²={r2_t:.4f}', color=GREEN); ax2.set_xlabel('True (G)'); ax2.set_ylabel('Pred (G)')
...
ax3.set_title('Error Dist', color=RED); ax3.legend(fontsize=8)
...
ax4.set_title('Error vs Range', color=PURPLE); ax4.set_xlabel('Range (km)')

ax5 = fig.add_subplot(gs[1, 1])
ax5.plot(range(1, 1001), history['grad_norms'], color=AMBER, alpha=0.5, linewidth=0.5)
ax5.set_title('Gradient Health', color=AMBER)
...
txt = (f"MODEL SUMMARY\n{'─'*30}\n"
       ...
       f"P95 Err: {sorted_err[int(0.95*len(sorted_err))]:.3f} G\n"
       f"P99 Err: {sorted_err[int(0.99*len(sorted_err))]:.3f} G\n"
       f"{'─'*30}\nDevice: {device}\nSamples: {N_SAMPLES:,}")
...
plt.savefig('viz20_final_dashboard.png', bbox_inches='tight'); plt.show()
print("  ✅ VIZ 20 done")
...
print("\n" + "=" * 70)
print(f"  📦 trained_missile_brain.pth ({model_size:.0f} KB)")
print(f"  Best Epoch: {best_epoch} | Val Loss: {best_val_loss:.6f}")
print(f"  R² Score: {r2_t:.4f}")
print(f"\n  📥 DOWNLOAD 'trained_missile_brain.pth' FROM KAGGLE OUTPUT")
print(f"     → Place it in: strike-vector/ai_training/trained_missile_brain.pth")
print(f"     → Select 'AI NEURAL NET' in the simulator dashboard")
print("=" * 70)
